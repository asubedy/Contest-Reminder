...
async def contestDisplayer(ctx, resource, name, image):
    ...
    parameters = {"limit": 4, "resource": f"{resource}", "order_by": "-start"}
...
async def leetcodeContest(ctx):
    # embedVar = discord.Embed(title="Upcoming Leetcode Contests",
    #                          description="These are the list of upcoming contest which are less than 7 days away", color=0x00ff00)
    await contestDisplayer(ctx, "atcoder.jp", "AtCcoder", "atcoder_jp")
    await contestDisplayer(ctx, "leetcode.com", "Leetcode", "leetcode_com")
    ...
    # await contestDisplayer(ctx, "leetcode.com", "leetcode_com")
